...
import com.qfedu.demo.service.PetService;
...
public class PetController {
    ...
    @GetMapping("/pet_list")
    public String list(String petName, String startTime, String endTime, Model model, @RequestParam(defaultValue = "1") Integer pageNum) {
        //查询单个或多个
        PageHelper.startPage(pageNum, 6);//pageNum:当前页码数，第一次进来时默认为1（首页）每页显示pagesize条
        List<Pet> pet = petService.query(petName, startTime, endTime);
        model.addAttribute("startTime",startTime);
        model.addAttribute("endTime",endTime);
        model.addAttribute("petName",petName);
        PageInfo<Pet> pageInfo = new PageInfo<>(pet);//pageInfo:将分页数据和显示的数据封装到PageInfo当中
    ...
    public String add(Model model) {
        List<Owner> ownerInfo = petService.queryOwnerInfo(null);
    ...
    @ResponseBody
    public RespBean insertPet(Pet pet) {
        pet.setPicture(imgString);
        Integer result = petService.insertPet(pet);
    ...
    public String edit(Model model, Integer id) {
        Pet petInfo = petService.queryById(id);
        List<Owner> ownerInfo = petService.queryOwnerInfo(null);
        model.addAttribute("ownerInfo", ownerInfo);
    ...
    @ResponseBody
    public RespBean Update(Pet pet) {
        RespBean respBean = null;
        pet.setPicture(imgString);
    ...
    @ResponseBody
    public RespBean deletePetById(@PathVariable("id") Integer id) {
    ...
    public RespBean deletePetByIds(@PathVariable("ids") String ids) {
        Boolean result = petService.deletePetByIds(ids);
        RespBean respBean = null;
        if (result) {
            respBean = RespBean.ok("删除成功");
        } else {
